...
def C_CF(u, t, r, nu, kappa, sigma, rho, bj, uj):
    d = np.sqrt( (rho*sigma*u*1j - bj)**2 - sigma**2 * (2*uj*u*1j - u**2) )
    g = (bj - rho*sigma*u*1j + d) / (bj - rho*sigma*u*1j - d)
    out = r*u*t*1j + (nu * kappa) / sigma**2 * ( (bj - rho*sigma*u*1j + d) * t - 2*np.log( (1 - g*np.exp(d*t)) / (1 - g)) );
    return out
...
def D_CF(u, t, sigma, rho, bj, uj):
    d = np.sqrt( (rho*sigma*u*1j - bj)**2 - sigma**2 * (2*uj*u*1j - u**2) )
    g = (bj - rho*sigma*u*1j + d) / (bj - rho*sigma*u*1j - d)
    out = (bj - rho*sigma*u*1j + d) / (sigma**2) * ((1 - np.exp(d*t)) / (1 - g*np.exp(d*t)))
    return out
...
def Call_Heston(K, T, r, nu, kappa, sigma, rho, S, V):
    ...
    integrand = lambda u: ( S*np.real(np.exp(-1j*u*alpha) * np.exp(C_CF(u, T, r, nu, kappa, sigma, rho, b1, u1)
                                                 + V * D_CF(u, T, sigma, rho, b1, u1) + 1j*u*x) / (1j*u)) 
                                                 - K * np.exp(-r*T) * real(np.exp(-1j*u*alpha) * np.exp(C_CF(u, T, r, nu, kappa, sigma, rho, b2, u2)
                                                 + V * D_CF(u, T, sigma, rho, b2, u2) + 1j*u*x) / (1j*u)) )
